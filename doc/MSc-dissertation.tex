...
\usepackage{graphics}
\usepackage{listings}
\usepackage{fullpage,epsf,graphicx, amstext,url,bm,amsmath, amssymb, caption, longtable} 
...
\begin{abstract}
This project details the onset of small-scale and large-scale dynamos (SSD and LSD) by numerically solving the incompressible MHD equations in a homogeneous isotropic scenario with the eDNS2017 code that uses pseudo-spectral methods in a triply periodic box. The performed simulations found an overall threshold magnetic Reynolds number ($Re_M^{th}=[39:50]$) (smallest $Re_M=u L /\eta$ to possibly sustain SSD) for the onset of SSD. Additionally, the critical magnetic Reynolds number for a fixed value of magnetic diffusivity ($\eta=0.0125$ was found to be the maximum $\eta$ able to sustain dynamo action) showed a dependence on the turbulence of the velocity field ($Re=u L/\nu$), and thus of $Pr_M=\nu/\eta$, which qualitatively matches~\cite{schekochihin2004critical}. This dependence was such that $Re_M^{crit}$ was larger for $Pr_M<1$ than for $Pr_M>1$ compared for a fixed $\eta$. A similar behaviour is expected for the LSD (mean-field theory of $\alpha$-effect shows this theoretically), but was not directly observed numerically, and is suggested as further work. 
\end{abstract}
...
All simulations were performed in the ARCHER supercomputer cluster at the Edinburgh Computing and Data Facility in the eDNS2017 code written for hydrodynamics by Dr. Sam Yoffe and extended to magnetohydrodynamics by Dr. Moritz Linkmann in C, who also wrote the post-processing scripts. 

All graphs, except one which was done with \texttt{Paraview}, shown in chapter~\ref{ch5} were done by simple \texttt{Bash} scripts calling \texttt{Gnuplot}.
...
The relation between the maximum wave number and the Kolmogorov microscales $k_{max}/k_\nu$ and $k_{max}/k_\eta$ is helpful to know how well resolved a given simulation is. Particularly, and as it was mentioned in section~\ref{sec4.2}, when $k_{max}/max(k_\nu, k_\eta) \geq 1.25$ then the simulation is said to be fully resolved, whereas if $1 < k_{max}/max(k_\nu, k_\eta) < 1.25$ a partially resolved simulation, and if $k_{max}/max(k_\nu, k_\eta) \leq 1$ the simulation is under resolved. Having said this, it is noted that in table~\ref{tableB1} all simulations for the S-runs (short) are fully resolved.
...
\begin{minipage}{.5\textwidth}
  \centering
  \captionsetup{justification=centering}
\includegraphics[width=0.95\textwidth]{{img/prm1/ss_prm_1_short}.png}
\caption{Temporal evolution of the total magnetic energy for simulations with ${Pr_M=1}$ from $t=60$ to $t=100$. These correspond to the S-runs in table~\ref{tableB1}.}
\label{fig5.12}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
  \captionsetup{justification=centering}
\includegraphics[width=0.95\textwidth]{{img/prm1/ss_prm_1_long}.png}
\caption{Temporal evolution of the total magnetic energy for simulations with ${Pr_M=1}$ from $t=60$ to $t=300$. These correspond to the L-runs in table~\ref{tableB1}.}
\label{fig5.13}
\end{minipage}
\end{figure}

Driven by the substantial difference between the total magnetic energy of S1 and S2/3, and because of doubts regarding having reached a real steady state in figure~\ref{fig5.12}, a set of runs with longer times was performed. This second set of simulations was done up to (at least) code time 300, but only for the magnetic diffusivity range $\eta=[0.01:0.0175]$. These simulations correspond to the L-runs (long) of table~\ref{tableB1}, and are plotted in the code time range 60 to 300 and shown in fig.~\ref{fig5.13}. It is clear that a transition from no dynamo action to dynamo action occurs somewhere between $\eta=0.015$ and $\eta=0.0125$. As a result, it is now possible to state that the critical magnetic Reynolds number lays somewhere in the range $Re_M^{crit}=[42:60]$. This is consistent with values found in the literature~\cite{brandenburg2005astrophysical, kazantsev1968enhancement}. A quick check of the L-runs in table~\ref{tableB1} shows that all the simulations used for fig.~\ref{fig5.13} are also fully resolved.

\subsection{Dynamo onset map}

After obtaining an estimate for the magnetic Reynolds number for unity $Pr_M$, and motivated in~\cite{schekochihin2004critical}'s discussion of the dependence of $Re_M^{crit}$ with $Re$, a larger set of runs was performed. The aim of this set of runs was to check what happened with the total magnetic energy at ``infinity'' through ensemble averages for values of $\nu$, that showed no dynamo action in the $Pr_M=1$ case, by varying the magnetic Prandtl number below and above unity. These simulations correspond to M-runs in table~\ref{tableB2}. There it can be checked that all simulations are appropriately resolved since for all simulations $min(k_{max}/k_\nu, k_{max}/k_\eta) > 1.25$.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.9\textwidth]{{img/map/3d_ss}.png}
\caption{Ensemble average of the total magnetic energy from $t=90$ to $t=100$ ($dt=0.1$) for all simulations in table~\ref{tableB2}.}
\label{fig5.14}
\end{figure}

Using the information of table~\ref{tableB2} a heat map was generated that shows the temporal average of the total magnetic energy in the range 60 to 100 of code time, every 0.1 units of code time, fig.~\ref{fig5.14}, where it is clear that for the higher values of magnetic diffusivity dynamo action cannot be sustained. This is a result of the magnetic field not being turbulent enough, i.e. the magnetic Reynolds number is too small. A heat map for the magnetic Reynolds number can be generated in a similar way than for $\langle E_M(t\rightarrow\infty) \rangle$, and it is shown in fig.~\ref{fig5.15}.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.9\textwidth]{{img/map/3d_ss_Rm}.png}
\caption{Magnetic Reynolds number for all simulations in table~\ref{tableB2}.}
\label{fig5.15}
\end{figure}

There is an obvious resemblance between fig.~\ref{fig5.14} and~\ref{fig5.15} because the dark parts of the $\langle E_M \rangle$ heat map, which did not achieve dynamo action, have magnetic Reynolds number smaller than $Re_M^{crit}(=[42:60]$ for $Pr_M=1)$. The shape of fig.~\ref{fig5.15} is not surprising at all since the magnetic Reynolds number is inversely proportional to $\eta$ ($Re_M=u_{rms}L/\eta$), so by decreasing the magnetic diffusivity the $Re_M$ increases.

Regarding fig.~\ref{fig5.14}, an interesting feature of the simulations that appear to have dynamo action when $Pr_M \sim 1$ ($\nu=[0.01, 0.0125, 0.015]$) is that no dynamo action is sustained when the magnetic Prandtl number is $Pr_M=0.2$. However, since in subsection~\ref{subsec5.3.1} it was seen that the simulations for $\eta=0.015\text{ and }0.0175$ did not reach a steady state after 100 units of code time, it is important to extend some of the simulations that appear to have dynamo action to longer times such that they reach a steady state and it is possible to confirm whether or not they sustain dynamo action.

The first thing that was done was generate two sets of longer runs to check what happens with the apparent dynamo action for $\eta=0.0125\text{ and }0.015$. These runs correspond to table~\ref{tableB3} and the total magnetic energy for different magnetic Prandtl numbers are shown in figs.~\ref{fig5.16} and~\ref{fig5.17} for $\nu=0.0125$ and $\nu=0.015$ respectively. The simulations with $\eta=0.0125, Pr_M=0.6$ and $\eta=0.015, Pr_M=1.2$ were run until $t=600$, and by that time both of them had completely lost their total magnetic energy due to dissipation. This leads to the statement that simulations with $\eta \geq 0.015$ cannot sustain dynamo action, and the largest magnetic Reynolds number of a simulation with $\eta=0.015$ is $Re_M=47\pm4$.
...
\begin{minipage}{.5\textwidth}
  \centering
  \captionsetup{justification=centering}
  \includegraphics[width=0.95\linewidth]{{img/map/long/ss_eta_0.0125}.png}
  \caption{Time evolution of the total magnetic energy for $\eta=0.0125$ and $Pr_M=[0.6, 0.8, 1, 1.2]$.}
  \label{fig5.16}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
  \captionsetup{justification=centering}
  \includegraphics[width=0.95\linewidth]{{img/map/long/ss_eta_0.015}.png}
  \caption{Time evolution of the total magnetic energy for $\eta=0.015$ and $Pr_M=[0.8, 1, 1.2]$.}
  \label{fig5.17}
\end{minipage}
\end{figure}

Regarding the $\eta=0.0125$ simulation, which is the largest magnetic diffusivity that is able to sustain dynamo action that was worked with, it is interesting to note that a small enough magnetic Prandtl number, large enough Reynolds number, is able to shut down dynamo action. The associated critical magnetic Prandtl number for this is somewhere in the range $Pr_M^{crit}=[0.6:0.8]$. Furthermore, it is possible to state that the critical magnetic Reynolds number for $Pr_M < 1$ in this case is somewhere in the range $Re_M^{crit}=[50:59]$. 
...
\begin{minipage}{.5\textwidth}
  \centering
  \captionsetup{justification=centering}
\includegraphics[width=0.95\textwidth]{{img/prm>1/ss_eta_0.0125_4-6-8}.png}
  \caption{Time evolution of the total magnetic energy for $\eta=0.0125$ and ${Pr_M=[4, 6, 8]}$ in the large range $t=[60:600]$.}
\label{fig5.18}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
  \captionsetup{justification=centering}
  \includegraphics[width=0.95\linewidth]{{img/prm>1/ss_eta_0.01}.png}
  \caption{Time evolution of the total magnetic energy for $\eta=0.01$ and $Pr_M=[20, 10, 7, 0.5, 0.2]$ in the range $t=[60:400]$.}
  \label{fig5.19}
\end{minipage}
\end{figure}

Afterwards, a similar treatment was done by keeping the magnetic diffusivity constant and set to $\eta=0.01$. These runs are summarised in table~\ref{tableB4}, and the exact same behaviour was observed when compared to the $\eta=0.0125$ case, or to~\cite{schekochihin2004critical, schekochihin2005onset}. The $E_M(t)$ for these simulations is plotted in fig.~\ref{fig5.19}. Particularly, critical magnetic Reynolds and Prandtl numbers were found for the cases with $Pr_M < 1$ and $Pr_M > 1$. Neither of these was equal to the values obtained for the $\eta=0.0125$. Particularly, the critical Prandtl number for simulations with $Pr_M < 1$ was found to be somewhere in the range $Pr_M^{crit}=[0.2:0.5]$, and for simulations with $Pr_M > 1$, $Pr_M^{crit}=[10:20]$. 

As far as the critical magnetic Reynolds number is concerned, for $Pr_M$ less than unity it is somewhere within $Re_M^{crit}=[54:69]$, while for $Pr_M>1$ it lays in the range ${Re_M^{crit}=[35:58]}$. For this smaller value of magnetic diffusivity, the difference between the critical magnetic Reynolds numbers from either side of $Pr_M=1$ is larger than for the $\eta=0.0125$ case. Consequently, it can be said more convincingly that turbulence in the velocity field (such that $Re > Re_M$, or $Pr_M<1$) leads to an increase in the critical magnetic Reynolds number. As it was mentioned before, the simulations in~\cite{schekochihin2004critical} are possibly under resolved due to the size of the boxes that they use for $\eta=0.001\text{ and }0.002$. However, the aforementioned behaviour for the critical magnetic Reynolds number is perfectly matched. Since the values they use for the magnetic diffusivity are considerably (one order of magnitude) smaller than the ones simulated with the eDNS2017 code, then it is not surprising to see that the difference they found in critical magnetic Reynolds numbers from either side of $Pr_M=1$ is considerably larger than the one documented here - $Re_M^{crit}\sim226$ for $Pr_M<1$ and $Re_M^{crit}\sim63$ for $Pr_M>1$~\cite{schekochihin2004critical}.

\section{Injection of kinetic helicity}

As it was mentioned in section~\ref{sec3.3}, the main difference between small-scale and large-scale dynamos is the presence of helicity in the system. Since the previous simulations were exclusively non-helical, then it is possible to state that they were showing the characteristics of small-scale dynamo action. In this section, the effect of injecting helicity into the system is studied, i.e. large-scale dynamo action is studied. 

A series of simulations with constant $\eta=0.01$ was generated by varying $Pr_M$ in the range $[0.2:50]$, and their information is shown in table~\ref{tableB5}. Additionally, $E_M(t)$ is plotted in the temporal range $t=[60:400]$ (fig.~\ref{fig5.20}). In the $Pr_M>1$ regime, the critical magnetic Prandtl number was found to be around $Pr_M^{crit}=[40:50]$, and the corresponding critical magnetic Reynolds number in the range $Re_M^{crit}=[22:31]$. This is expected theoretically from the mean-field description of the $\alpha$-effect, i.e. at least some turbulence in the velocity field is necessary to activate the $\alpha$-effect~\cite{schnack2009lectures}. A small enough $Pr_M$ was not found such that it shut down the large-scale dynamo. However,~\cite{schnack2009lectures}, on purely analytic grounds, shows that a too large value of $Re$ (too much turbulence in the velocity field) must also shut down the $\alpha$-effect, and the large-scale dynamo with it. 

\begin{figure}[!ht]
\centering
\includegraphics[width=0.7\textwidth]{{img/helical/ss_eta_0.01}.png}
\caption{Time evolution of the total magnetic energy for helically forced simulations with $\eta=0.01$ and $Pr_M=[50, 40, 30, 0.2]$ in the range $t=[60:400]$, table~\ref{tableB5}.}
\label{fig5.20}
\end{figure}

\section{Small-scale dynamo?}

Extending fig.~\ref{fig5.10} to simulations with $Pr_M<1$ showed very interesting spectral behaviour for the magnetic energy. It is clear that in fig.~\ref{fig5.10} all simulations show the characteristic Kazantsev scaling except for the one that does not sustain dynamo action, $Pr_M=100$. Nonetheless, all simulations show a similar spectral profile for the magnetic energy in the intermediary and large wave numbers. In fig.~\ref{fig5.21}, that shows the $Pr_M<1$ simulations along with those of fig.~\ref{fig5.10}, it is interest to note the difference in the spectral behaviour between simulations with $Pr_M \geq 1$ and $Pr_M < 1$. 

\begin{figure}[!ht]
\centering
\includegraphics[width=0.7\textwidth]{{img/kazantsev_0.01}.png}
\caption{Runs with $Pr_M=[0.1:100]$, showing a clear difference in behaviour for $Pr_M\geq1$ and $Pr_M<1$.}
\label{fig5.21}
\end{figure}

Comparing the simulations with $Pr_M=1\text{ and }0.5$, both of which sustain dynamo action, shows that the magnetic energy spectrum for the latter is more energetic at the larger wave numbers. The same behaviour is found when comparing all simulations with the $Pr_M=0.1$ one, with the clear difference that all (except $Pr_M=100$) compared simulations sustain dynamo action. This substantial difference could be related to remarks in~\cite{schekochihin2004critical} that state that only large-$Pr_M$ non-helical simulations show small-scale dynamo action. A thorough investigation into this difference would be definitely interesting further work and, if successful, might help elucidate exactly what are the small-$Pr_M$ non-helical systems.

\chapter{Conclusions}

This project was based on studying the onset of small-scale and large-scale dynamo action by performing numerical simulations in the eDNS2017 code, written by Yoffe~\cite{yoffe2013investigation} and extended by Linkmann~\cite{LinkmannMoritzFrederikLeon2016Spim}, to solve the incompressible MHD equations (eqns.~\ref{eq3.1}-\ref{eq3.3}) in a scenario of homogeneous isotropic turbulence inside a triply periodic box using the pseudo-spectral method common to DNS. All simulations that were carried out applied forcing to the velocity field $\bm f_u$. For most simulations the forcing that was used was of a non-helical nature. By embracing the definition of~\cite{tobias2011mhd} for small-scale and large-scale dynamo action as those which are non-helical and generating spatially small magnetic fields, and helical and generating spatially coherent large magnetic fields, respectively, it is possible to characterise the simulations that were run. 

By first making consistency checks that the eDNS2017 code gave the appropriate physics of known analytic behaviours a ``verification'' stage was performed. This included checking that the fields exhibited a sort of duality between themselves (this was done by showing two dimensional slices of $\vert \bm u \vert\text{ and }\vert \bm b \vert$ using \texttt{Paraview}), observing the effects of under resolved simulations in the energy and energy dissipation temporal evolutions, confirming that Kolmogorov $k^{-5/3}$ scaling (for cases where the magnetic energy spectrum was comparatively weak to the kinetic spectrum) was found to fit at intermediary length scales for several simulations (fig.~\ref{fig5.8}). Additionally, it was confirmed that for cases with sustained small-scale dynamo action (non-helical) the Kazantsev $k^{3/2}$ scaling was met when $Pr_M \geq 1$, as required by~\cite{kazantsev1968enhancement, haugen2004simulations}. 
