...
A detailed account of the simulations that were generated, along with their motivations, is provided here. The first simulation was done in a large box, $N^3=1024^3$, for which viscosity ($\nu=0.005$) was ten times larger than magnetic diffusivity, i.e. $Pr_M=\nu/\eta=10$. As it was mentioned before, the magnetic Prandtl number compares the kinetic and the magnetic channel. That is, it compares how turbulent the magnetic field is relative to the velocity field, which is equivalent of comparing how much energy is dissipated through the magnetic channel against the kinetic channel. 

For the simulation with $N^3=1024^3$ grid points, \texttt{Paraview}, a slice of the velocity and the magnetic fields' magnitude was plotted. Figure~\ref{fig5.1} shows this slice, where the magnetic field is clearly more fine-grained than the velocity field. Which is not surprising since the Kolmogorov microscales for this system are $k_\nu = 33$ and $k_\eta = 185$.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.9\textwidth]{{img/f_1024_0.005_10_fieldslice}.png}
\caption{Heat map of $\vert u \vert$ (left) and $\vert b \vert$ (right) of a vertical  slice of a $N^3=1024^3$ box ~\cite{schekochihin2004simulations}.}
\label{fig5.1}
\end{figure}
