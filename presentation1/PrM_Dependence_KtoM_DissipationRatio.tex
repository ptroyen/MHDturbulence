...
\begin{frame}
 ...
 \begin{itemize}
  ...
  %\item<3-> Assume continuous flow.% $\partial_t \bm{E} = \bm{0}$.
...
\begin{frame}
 ...
 %$Re \sim 40 \text{ to} \sim 1000$

 
 \transduration<0-5>{0}
 \multiinclude[format=png, end=5, graphics={width=\textwidth}]{img/vortex}
...
\begin{frame}
 ...
 %\begin{align}
  %&s=1 \qquad y_{n+1} = y_n + h f(t_n, y_n)\qquad\text{Euler integration} \nonumber \\
  %&s=2 \qquad y_{n+2} = y_{n+1} + h \left(\frac{3}{2}f(t_{n+1}, y_{n+1})-\frac{1}{2}f(t_n, y_n)\right) \nonumber \\
  %&s=3 \qquad y_{n+3} = y_{n+2} + h \left(\frac{23}{12}f_{n+2}^{n+2}-\frac{4}{3}f_{n+1}^{n+1} + \frac{5}{12}f_n^n \right) \nonumber
 %\end{align}
\end{frame}
...
\bibliography{PrM_Dependence_KtoM_DissipationRatio}
